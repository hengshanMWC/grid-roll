  ...
  methods: {
    ...
    setIndex (indexs) {
      ...
      if (Array.isArray(indexs)) {
        this.indexs = JSON.parse(JSON.stringify(indexs))
      } else {
        this.indexs = [ indexs ]
      }
    },
    dischargeCargo () {
      if (!isNaN(Number(this.indexValue))) {
        this.underway(this.countStep(this.indexValue))
      }
    },
    /**
     * 计算步数
    ...
    */
    underway (number, status = true) {
      ...
      // 没有步数
      if (number <= 0) {
        this.$emit('select', this.indexValue, this.index)
        this.reincarnation()
        return
      }
    ...
    reincarnation () {
      // 看看是否还有
      if (this.indexs.length > ++this.index) {
        this.dischargeCargo()
      } else {
        // 抽奖全部完成
        this.resolve(true)
